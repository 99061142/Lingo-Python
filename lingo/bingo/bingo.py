...
def play_bingo_round_for_team(team_ID: int) -> None:
    ...
    for attempt in range(1, max_grab_attempts + 1):
        ...
        if grabbed_ball == "red":
            # If the team has grabbed enough red balls to lose the bingo game, we print the lose message, set the current team as the losing team, and return
        ...
        # If the team has grabbed enough green balls to win the bingo game, we print the win message, set the current team as the winning team, and return
        green_balls_needed_to_win = win_conditions["green_balls_grabbed"]
        if get_amount_of_color_balls_grabbed_by_team(team_ID, "green") >= green_balls_needed_to_win:
